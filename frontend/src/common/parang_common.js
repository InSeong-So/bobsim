const parangApp = {
  ...
  getUniqueArray: function (duplicateArray, type) {
    ...
    //---------
    // let uniq = names.slice() // 정렬하기 전에 복사본을 만든다.
    //     .sort(function(a, b) {
    //         return a - b;
    //     })
    //     .reduce(function(a, b) {
  ...
  mergeTableTr: function () {
    ...
    $('tr[dynamicRowspan]').each(function (row) {
      ...
      if (row > 2) {
        let tr = $(this);
        let element = $(':first-child', tr).html();
...
let PerformanceCheck = {
  start() {
    let setTestFunc = setTimerFunc((a, b) => a + b, [1, 2], loopCount);
    let loopLoopCount = 10;
    let loopTestTime = getLoopTestTime(setTestFunc, loopLoopCount);
  ...
  testTimeReport(costTimes) {
    let copied = costTimes.slice(); // 값만 복사해서 원본의 훼손을 막습니다
    return {
